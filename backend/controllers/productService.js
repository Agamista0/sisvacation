...
const getProductDesc = (req, res) => {
  const { offerId } = req.params;
  console.log(offerId);
  const query = 'SELECT description FROM offer WHERE id = ?';
  db.query(query, [offerId], (err, results) => {
    if (err) {
...
const getProducts = (req, res) => {
  ...
  const sql = `
    SELECT o.*, 
           CASE WHEN w.product_id IS NOT NULL THEN true ELSE false END AS wishlisted
    FROM offer o
    LEFT JOIN wishlist w ON o.id = w.product_id AND w.session_id = ?
  `;
...
const getProductsLocation = (req, res) => {
  ...
  // Join with the wishlist table to include the wishlisted status
  const sql = `
    SELECT o.*, 
           CASE WHEN w.product_id IS NOT NULL THEN true ELSE false END AS wishlisted
    FROM offer o
  ...
  db.query(sql, [sessionId, location], (err, results) => {
    if (err) {
      console.error('Error fetching products:', err);
      res.status(500).send('Error fetching products');
    } else {
...
const deleteCartItem = (req, res) => {
 ...
 export { getProducts, addToCart, getProductsCart, getCartCount, deleteCartItem, updateCartItem, getProduct, getProductsLocation, getProductDesc };
